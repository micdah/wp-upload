...
function findTmpdirMount(fsSizes: Awaited<ReturnType<typeof si.fsSize>>): (typeof fsSizes)[number] | null {
...
async function sample(): Promise<void> {
  ...
  if (memResult.status === 'fulfilled') {
    ...
    memory = { totalBytes: mem.total, usedBytes: mem.active, usedPercent: (mem.active / mem.total) * 100 };
  } else {
  ...
  if (diskResult.status === 'fulfilled') {
    ...
    disk = match ? { mount: match.mount, totalBytes: match.size, usedBytes: match.used, usedPercent: match.use } : null;
  } else {
    console.warn('serverLoad: failed to sample disk usage', diskResult.reason);
  }

  let network: ServerLoadSnapshot['network'] = latestSnapshot?.network ?? { iface: null, rxBytesPerSec: null, txBytesPerSec: null };
  if (netResult.status === 'fulfilled') {
    const iface = netResult.value[0];
    network = iface
      ? { iface: iface.iface, rxBytesPerSec: iface.rx_sec, txBytesPerSec: iface.tx_sec }
      : { iface: null, rxBytesPerSec: null, txBytesPerSec: null };
  } else {
    console.warn('serverLoad: failed to sample network throughput', netResult.reason);
  }

  if (cpuResult.status === 'rejected') console.warn('serverLoad: failed to sample CPU usage', cpuResult.reason);
  ...
  latestSnapshot = {
    ...
    disk,
    network,
    loadAverage: { '1m': loadavg[0] ?? 0, '5m': loadavg[1] ?? 0, '15m': loadavg[2] ?? 0 },
...
export function startServerLoadSampler(): void {
  setInterval(() => {
    sample().catch((err) => console.error('serverLoad: unexpected sampling failure', err));
  }, SAMPLE_INTERVAL_MS);
}
