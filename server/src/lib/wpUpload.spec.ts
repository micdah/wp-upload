...
describe.concurrent('normaliseWpError', () => {
  it('maps ECONNABORTED to a 504 timeout error', () => {
    const result = normaliseWpError({ isAxiosError: true, code: 'ECONNABORTED' });
  ...
  it('passes through a JSON error body from WordPress', () => {
    const result = normaliseWpError({
      isAxiosError: true,
      response: { status: 400, data: { code: 'rest_invalid_param', message: 'Bad request' } },
    });

    expect(result).toEqual({ status: 400, code: 'rest_invalid_param', message: 'Bad request' });
  });
  ...
  it('falls back to "wp_error" when the JSON body has a message but no code', () => {
    ...
    expect(result).toEqual({ status: 400, code: 'wp_error', message: 'Bad request' });
  });
  ...
  it('classifies a non-axios Error as unreachable using its message', () => {
    ...
    expect(result).toEqual({ status: 502, code: 'wp_unreachable', message: expect.stringContaining('boom') });
  });
...
describe.concurrent('isWpError', () => {
  ...
  it.each([null, undefined, 'string', 42, {}, { status: 1 }, { status: 1, code: 'x' }])(
    'rejects %j',
    (value) => {
...
describe('uploadToWordPress', () => {
  ...
  beforeEach(() => {
    vi.spyOn(fs, 'createReadStream').mockReturnValue({} as unknown as ReadStream);
  });
  ...
  it('falls back to the original filename when WordPress omits a title', async () => {
    vi.spyOn(wpAxios, 'post').mockResolvedValue({
      data: { id: 7, source_url: 'https://example.invalid/photo.png', mime_type: 'image/png' },
    } as any);
  ...
  it('invalidates the connection and rethrows the normalised error on a 401/403', async () => {
    vi.spyOn(wpAxios, 'post').mockRejectedValue({
      isAxiosError: true,
      response: { status: 401, data: { code: 'rest_forbidden', message: 'Sorry, you are not allowed.' } },
    });
    ...
    expect(connectionState.reason).toBe('WordPress rejected the configured credentials.');
  });
