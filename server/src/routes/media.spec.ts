...
describe('media', () => {
  ...
  describe('POST /media', () => {
    ...
    it('responds 400 when no file is attached', async () => {
      ...
      expect(res.body).toEqual({ code: 'no_file', message: 'No file was provided.' });
    });
    ...
    it('responds 201 with the mapped result on success', async () => {
      ...
      const res = await request(app).post('/media').attach('file', Buffer.from('fake-image-bytes'), 'photo.png');
    ...
    it('maps a WpError from uploadToWordPress to the matching HTTP response', async () => {
      ...
      expect(res.body).toEqual({ code: 'wp_unreachable', message: 'Could not reach WordPress.' });
    });
  });
});
