...
describe.concurrent('basicAuth', () => {
  ...
  it('rejects malformed credentials with no ":" separator', () => {
    const { req, res, next } = createReqRes('10.0.0.3', `Basic ${Buffer.from('no-colon-here').toString('base64')}`);
  ...
  it('rejects a wrong username or password', () => {
    const { req, res, next } = createReqRes('10.0.0.4', basicAuthHeader('wrong', 'test-auth-password'));
  ...
  it('calls next() and clears prior failures on correct credentials', () => {
    ...
    const { req, res, next } = createReqRes(ip, basicAuthHeader('test-auth-user', 'test-auth-password'));
    basicAuth(req, res, next);
  ...
  it('opportunistically sweeps other IPs\' stale entries on every request', () => {
    ...
    try {
      ...
      const { req, res, next } = createReqRes(ipC, basicAuthHeader('test-auth-user', 'test-auth-password'));
      basicAuth(req, res, next);
